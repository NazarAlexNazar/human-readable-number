module.exports = function toReadable (number) {
  ...
  let arrDec = ['twenty', 'thirty', 'forty', 'fifty', 'sixty', 'seventy', 'eighty', 'ninety'];
  let strText = '';
  let numStr = String(number);

  if (number > 99) {
    strText += arr[Number(numStr.substr(-3,1))]  + ' hundred ';
    numStr = numStr.substr(-2);
  }

  if (Number(numStr) > 19) {
    strText += arrDec[Number(numStr[0]) - 2] + ' ';
    numStr = (numStr[1] === '0') ? '' : numStr[1];
  } else if (numStr === '00') {
    numStr = '';
  }

  strText += (numStr === '') ? '' : arr[Number(numStr)];
